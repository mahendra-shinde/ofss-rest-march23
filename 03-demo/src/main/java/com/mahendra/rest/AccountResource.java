...
public class AccountResource {
	...
	/*
	 * @GET public Response findAccount(@QueryParam("accnum") String accNum) {
	 * Response response = null; if(accNum.equalsIgnoreCase("SB1111")) { Account acc
	 * = new Account("Chandrachud", "SB1111", "Savings"); response =
	 * Response.ok(acc).build(); }else { response =
	 * Response.status(404).header("error", "Account not found ").build(); } return
	 * response; }
	 */	
	
	@POST @Consumes("application/json")
	public String create(Account acc) {
		System.out.println("Creating "+acc.getAccNum());
		dao.add(acc);
		return "Created";
	}
